...
class School:
    ...
    def get_tables(self):
        with requests.session() as session:
            ...
            logging.debug(f"Fetching the main page for the school {self.name}...")
            ...
            logging.debug(f"Getting the page with list of tables...")
            ...
            print(f"{len(soup.find_all(title='Concentrado de Información'))} tables found!")
            ...
            for count, table_page_element in enumerate(soup.find_all(title="Concentrado de Información")):
                ...
                logging.debug(f" Going to {table_url} to fetch the table...")
                table_page = session.get(table_url, headers=headers)
                logging.debug(f"Response status: {table_page.status_code}")
                ...
                logging.debug(f"Downloading the table...")
